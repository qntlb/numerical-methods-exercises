...
class ExchangeOptionTest {
	...
	public void testExchangeWithRandomSeeds() throws CalculationException {
		...
		final AssetModelMonteCarloSimulationModel firstSimulationTwoDimGeometricBrownian = new
				MonteCarloMultiAssetBlackScholesModel(
						brownian, initialPrices, riskFreeRate, volatilities,
						correlationMatrix);

		ExchangeOption exchangeOption=new ExchangeOption(maturity);
		...
		double error = Math.abs(monteCarloPrice - analyticalPrice)/analyticalPrice*100;
		...
		for (int i=0; i< numberOfRepetitions - 1; i++) {
			...
			final AssetModelMonteCarloSimulationModel simulationTwoDimGeometricBrownian =
					firstSimulationTwoDimGeometricBrownian.getCloneWithModifiedSeed(randomSeed);

			exchangeOption=new ExchangeOption(maturity);

			monteCarloPrice = exchangeOption.getValue(simulationTwoDimGeometricBrownian);
			analyticalPrice = computeAnalyticalValue();
			error = Math.abs(monteCarloPrice - analyticalPrice)/analyticalPrice*100;
