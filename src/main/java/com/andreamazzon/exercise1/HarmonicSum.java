 ...
 * identical to the value of the larger number. This is the issue it happens
 * when the forward computation is performed. You may also note that methods are
 * NOT identified by the returning type: we have two methods that return the
 * value of the forward harmonic sum, but one returns it as a float, the other
 * as a double: they have to be named differently, otherwise we get a compile
 * time error.
 */
